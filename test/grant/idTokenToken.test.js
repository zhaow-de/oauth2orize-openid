...
var idTokenToken = require('../../lib/grant/idTokenToken');
  ...
  describe('request parsing', function() {
    ...
    describe('request', function() {
      ...
      before(function(done) {
        chai.oauth2orize
          .grant(idTokenToken(issueToken, issueIDToken))
          .req(function(req) {
            req.query = {};
    ...
    describe('request with list of scopes using scope separator option', function() {
      ...
      before(function(done) {
        chai.oauth2orize
          .grant(idTokenToken({ scopeSeparator: ',' }, issueToken, issueIDToken))
          .req(function(req) {
            req.query = {};
            req.query.client_id = 'c123';
            req.query.redirect_uri = 'http://example.com/auth/callback';
  ...
  describe('issuing an access token', function() {
    describe('based on client, user, and authorization response', function() {
      ...
      it('should respond', function() {
        ...
        expect(response.getHeader('Location')).to.equal(
          'http://example.com/auth/callback#access_token=xyz&token_type=Bearer&id_token=idtoken'
        );
      });
    });
    ...
    describe('based on client, user, and authorization response, that adds additional parameters', function() {
      ...
      it('should respond', function() {
        expect(response.statusCode).to.equal(302);
        expect(response.getHeader('Location')).to.equal(
          'http://example.com/auth/callback#access_token=xyz&expires_in=3600&token_type=Bearer&id_token=idtoken'
        );
      });
    });
    ...
    describe('based on client, user, and authorization response, that adds additional parameters including token_type', function() {
      ...
      it('should respond', function() {
        expect(response.statusCode).to.equal(302);
        expect(response.getHeader('Location')).to.equal(
          'http://example.com/auth/callback#access_token=xyz&token_type=foo&expires_in=3600&id_token=idtoken'
        );
      });
    });
    ...
    describe('based on client, user, and authorization response, while preserving state', function() {
      ...
      it('should respond', function() {
        expect(response.statusCode).to.equal(302);
        expect(response.getHeader('Location')).to.equal(
          'http://example.com/auth/callback#access_token=xyz&token_type=Bearer&state=f1o1o1&id_token=idtoken'
        );
      });
    });
