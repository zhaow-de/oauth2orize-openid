 ...
 * Module dependencies.
 */
var AuthorizationError = require('@zhaow-de/oauth2orize').AuthorizationError;
...
module.exports = function() {
  function request(req) {
    ...
    if (q.prompt) {
      ext.prompt = parse('prompt', true);
    }
    if (q.max_age) {
      ext.maxAge = parseInt(q.max_age);
    }
    if (q.ui_locales) {
      ext.uiLocales = parse('ui_locales', true);
    }
    if (q.claims_locales) {
      ext.claimsLocales = parse('claims_locales', true);
    }
    ext.idTokenHint = q.id_token_hint;
    ext.loginHint = q.login_hint;
    if (q.acr_values) {
      ext.acrValues = parse('acr_values', true);
    }
